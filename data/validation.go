...
// Validate the item for more detail
// the returned error can be cast into a
...
func validateSKU(fl validator.FieldLevel) bool {
	// SKU must be in the format 'abc-abc-123'
	rex := regexp.MustCompile(`[a-z]{3}-[a-z]{3}-[\d]{3}`)
	skuFormatMatch := rex.FindAllString(fl.Field().String(), -1)

	return len(skuFormatMatch) == 1
}
